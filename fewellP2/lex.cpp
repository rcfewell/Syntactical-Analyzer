...
Lex::~Lex()
{
	input.close ();
	//output.close();
	// cout << errors << " erros found in input file\n";
}
...
token_type Lex::get_token ()
{
	...
	while( isspace( line[pos] ) )
	{
		if( ++pos >= line.length() )
		{
			...
			output << setw( 4 ) << linenum << ": " << line << endl;
			line += ' ';
			pos = 0;
		}
	}
...
string Lex::get_token_name( token_type t ) const
{
	return token_names[t];
}

string Lex::get_lexeme () const
{
	return lexeme;
}

void Lex::error_message( string msg )
{
	output << "Error at " << linenum << ',' << pos << ": " << msg << endl;
